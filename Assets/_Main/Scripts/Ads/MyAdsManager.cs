...
//	GameObject simpleBanner,adaptiveBannerScreen;
	...
	IEnumerator Start()
	{
		...
		InitializeAds();
        StartCoroutine(ShowBannerRountine());//husasin
	...
	{
		if (!isInternetAvailble() || !canShowAds || SceneManager.GetActiveScene().buildIndex == 0/*|| AdsRemoteConfigManager.Instance.IsHideAd()*/) 
			return;
	...
	{
		if (Application.internetReachability == NetworkReachability.NotReachable ||  (PlayerPrefs.GetInt("NoAds") == 1) || !canShowAds || SceneManager.GetActiveScene().buildIndex == 0)
			return;
